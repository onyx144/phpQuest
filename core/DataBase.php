...
class DataBase
{
    ...
    private static $db = null; // Единственный экземпляр класса
    private $mysqli; // Идентификатор соединения
    private $sym_query = "{?}"; // "Символ значения в запросе"

    /* Получение экземпляра класса. Если он уже существует, то возвращается,
    ...
    /* Вспомогательный метод, который заменяет "символ значения в запросе"
     * на конкретное значение, которое проходит через "функции безопасности" */
        ...
        if ($params) {
            for ($i = 0; $i < count($params); $i++) {
                ...
                if (!is_null($params[$i])) {
                    ...
                    $query = substr_replace($query, "'NULL'", $pos, strlen($this->sym_query));
                }
            }
        }
        return $query;
