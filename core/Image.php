...
final class Image {
	...
	// конструктор - создаем новое фото для дальнейшей работы
		...
		if (file_exists($file)) {
        	...
        	if (function_exists('exif_read_data') && $info['mime'] == 'image/jpeg') {
				...
				if (!empty($exif_data['Orientation'])) {
					...
					if ($exif_orientation == 3 || $exif_orientation == 6 || $exif_orientation == 8){
						$new_angle = [
							3 => 180,
							6 => -90,
							8 => 90
						];
						$this->image = imagerotate($this->image, $new_angle[$exif_orientation], 0);

						// если поворот на 90 градусов в любом направлении, то меняем местами ширину и высоту
						if ($exif_orientation == 6 || $exif_orientation == 8) {
							$additional_variable = $this->info['width'];
							$this->info['width'] = $this->info['height'];
							$this->info['height'] = $additional_variable;
						}
					}
				}
        	}
	...
	// вспомогательная функция для создания фото
    ...
    // преобразование фото к новым размерам (дефолтная функция. Почему-то не подошла...)
    ...
    // преобразование фото к новым размерам
    public function resize1($width = 0, $height = 0, $default = '') {
		if (!$this->info['width'] || !$this->info['height']) {
			return;
		}
    ...
    // обрезка фото по одной стороне (вроде?)
    ...
    // преобразование фото к новым размерам по одной максимальной стороне
    ...
    // обрезаем часть фото
    public function crop($top_x, $top_y, $width_x, $height_y)
    {
        $image_old = $this->image;
        $this->image = imagecreatetruecolor($width_x, $height_y);
        ...
        $this->info['width'] = $width_x;
        $this->info['height'] = $height_y;
    }
