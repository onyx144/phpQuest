...
/* ОБЩИЕ ФУНКЦИИ */
	...
	function databasePersonalFilesPrivateIndividualsNoEmptyFields(firstname, lastname, lang_abbr2) {
		...
		searchInterval = setInterval(function(){
			...
			if (next == 100) {
		    	...
		    	formData.append('op', 'validatePersonalFilesPrivateIndividualsSearch');
				...
				$.ajax({
					...
					success: function(json) {
						...
						if (json.success) {
							...
							// грузим новый таб
							uploadTypeTabsDatabasesStep('databases_start_four_inner_second_personal_files_private_individual_huilov', 'car_register', false);
						} else {
...
$(function() {
	...
	$('body').on('click', '.dashboard_personal_files1_category_private_individuals', function(e){
		$.when(getTeamInfo()).done(function(teamResponse){
			...
			if (teamInfo) {
				if (teamInfo.private_individuals_print_text_huilov == 1) { // если нашли уже
					uploadTypeTabsDatabasesStep('databases_start_four_inner_second_personal_files_private_individual_huilov', 'personal_files', true);
				} else { // экран поиска
					uploadTypeTabsDatabasesStep('databases_start_four_inner_second_personal_files_private_individual', 'personal_files', true);
				}
			}
		});
	});
	...
	// действие при вводе символов в поля ввода данных
	...
	// personal files - private individuals search. Нажали на кнопку поиска
	$('body').on('click', '.dashboard_personal_files2_private_individuals_search', function(e){
		...
		if (!err) {
			...
			$.when(getTeamInfo()).done(function(teamResponse){
				...
				scoreBeforeDatabasePersonalfilesPrivateindividuals = parseInt(teamInfo.score, 10);
				...
				var message = {
					...
					'parameters': {
						...
						'scoreBeforeDatabasePersonalfilesPrivateindividuals': scoreBeforeDatabasePersonalfilesPrivateindividuals,
						'user_id': $('#section_game').length ? $('#section_game').attr('data-user-id') : 0,
						'team_id': $('#section_game').length ? $('#section_game').attr('data-team-id') : 0
					}
		        };
				...
				databasePersonalFilesPrivateIndividualsNoEmptyFields(firstname, lastname, $('html').attr('lang'));
			});
		} else {
			// socket
			var message = {
